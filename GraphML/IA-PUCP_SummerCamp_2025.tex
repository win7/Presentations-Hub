...
% \begin{comment}
    ...
    \begin{frame}{Based on edges}         
        \begin{exampleblock}{Undirected graph}
            Is a graph $G = (V, E)$ in which edges have no direction. The edge $(u, v)$ is equivalent to $(v, u)$. 
            % That is, an edge between two vertices uu and vv is represented as the unordered pair {u,v}{u,v}, and it implies a bidirectional relationship between the vertices.
            % E⊆{{u,v}∣u,v∈V,u=v} is a set of unordered pairs (edges)
        ...
        Example: commonly used to model symmetric relationships, such as friendships, road connections (two-way streets), etc.
    ...
    \begin{comment}
    \begin{frame}{Based on edges}         
        \begin{exampleblock}{Unweighted graph}
            Is a graph $G = (V, E)$ in which all edges are considered equal, which means that no numerical values are associated with them.
            
            \begin{figure}
                \centering
                \begin{subfigure}{0.4\textwidth}
                    \includegraphics[width=\textwidth]{pic/homogeneous_graph}
                    \caption{Unweighted \& undirected.}
                    \label{fig:unweighted_undirected}
                \end{subfigure}
                % \hfill
                \hspace{1cm}
                \begin{subfigure}{0.4\textwidth}
                    \includegraphics[width=\textwidth]{pic/directed_graph.pdf}
                    \caption{Unweighted \& directed.}
                    \label{fig:unweighted_directed}
                \end{subfigure}
                \caption{Unweighted graph.}
                \label{fig:unweighted_graph}
            \end{figure}
        \end{exampleblock}
        \vspace{-2mm}

        Example: commonly used in social networks (friend/not friend), basic graph traversal, etc.
    ...
    \begin{comment}
    \begin{frame}{Graph-level}
        \begin{exampleblock}{Diameter}
            Given a graph $G = (V, E)$.
            
            The diameter is the longest shortest path distance between any two nodes in the graph. It represents the maximum eccentricity.

            % The goal of network diameter measure is to index the extensiveness of the network, which means how far apart the two furthest nodes in the network are from each other.

            \begin{equation}
                diameter(G) = {\max_{v \in V} \epsilon(v)}
                \label{eq:diameter}
            \end{equation}
        \end{exampleblock}
        % \vspace{-5mm}
        ...
        Applications:
        \begin{itemize}
            \item Computer networks: estimate the worst-case latency.
            \item Transportation networks: maximum travel time between two points.
            \item Biology: longest signaling distance in cellular or neural networks.
            \item Social networks: measure how "small" the world is.
        \end{itemize}
    ...
    \begin{frame}{Graph-level}
        \begin{exampleblock}{Radius}
            Given a graph $G = (V, E)$.
            
            The radius is the minimum path length between any two nodes. It represents the minimum eccentricity.

            \begin{equation}
                radius(G) = {\min_{v \in V} \epsilon(v)}
                \label{eq:radius}
            \end{equation}
        \end{exampleblock}
        % \vspace{-5mm}
        ...
        Applications:
        \begin{itemize}
            \item Network design: find optimal central location (e.g., data center, hospital).
            \item Social networks: identify most reachable or influential users.
            \item Biology: locate central molecules in metabolic networks.
        \end{itemize}
    ...
    \begin{frame}{Practical example: Graphs and visualization} 
        \url{https://drive.google.com/file/d/199QkU5jbJrrg4URuxGU8wGuvIF91y1mo/view?usp=sharing}
    \end{frame}
...
\subsection{Shallow embeddings}
    ...
    \begin{frame}{Practical example: Node2vec} 
        \url{https://drive.google.com/file/d/1FkXYVMxq9TH_57hfl3nBMGsVH9fwZ6dC/view?usp=sharing}
    \end{frame}
