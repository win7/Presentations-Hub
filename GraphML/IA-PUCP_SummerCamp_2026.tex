...
\usepackage[skip=5pt]{caption} % o skip=0pt to zero space
...
\author[IA-PUCP]{Artificial Intelligence Summer Camp}

\title{Graph Machine Learning}
% \subtitle{Presentation}
\institute [Artificial Intelligence Summer Camp] 
{
\vspace*{-0.1cm}
...
\date{\scriptsize January 15, 2026}
\usepackage{YTU}
...
\begin{frame}{What do you see?}
    \begin{figure}[ht!]
            \centering
            \includegraphics[width=0.5\linewidth]{pic/facebook_nonames1-300x297.jpg}
...
\section{Graph theory}
    ...
    \begin{frame}{Graphs are everywhere!}
        \begin{figure}
            ...
            \begin{subfigure}{0.35\textwidth}
                \includegraphics[width=\textwidth]{pic/social_networks.png}
                \caption{Social networks.}
                %%% \label{fig:social_networks}
            \end{subfigure}
            % \hfill
            \begin{subfigure}{0.35\textwidth}
                \includegraphics[width=\textwidth]{pic/road_map_landmarks.png}
                \caption{Road-map of landmarks.}
                %%% \label{fig:road_map_landmarks}
            \end{subfigure}
            % \hfill
            \begin{subfigure}{0.35\textwidth}
                \includegraphics[width=\textwidth]{pic/financial_networks.png}
                \caption{Financial networks.}
                %%% \label{fig:financial_networks}
            \end{subfigure}
            % hfill
            \begin{subfigure}{0.35\textwidth}
                \includegraphics[width=\textwidth]{pic/personnel_assignment.png}
                \caption{Assignment of employees to jobs.}
    ...
    \begin{frame}{Node-level}
        \begin{exampleblock}{Average degree}
            ...
            \begin{multicols}{2} % two columns
                ...
                The average degree of a graph satisfies the properties given in Equation \ref{eq:degree_distribution}.
                \begin{equation}
                    deg(G) = \frac{2|E|}{|V|}
    ...
    \begin{frame}{Practical example: Graphs and visualization}
        \begin{itemize}
...
\section{Graph machine learning}
    \subsection{Preliminaries}
        ...
        \begin{frame}{Neural networks (NN)}
            \begin{figure}[ht!]
                ...
                \vspace{-15pt}
                \caption*{\tiny From: \url{https://medium.com/data-science-365/overview-of-a-neural-networks-learning-process-61}}
            \end{figure}   
        \end{frame}
        ...
        \begin{frame}{Example of NN}
            \begin{figure}
                ...
                \vspace{-15pt}
                \caption*{\tiny From: \url{https://www.3blue1brown.com/lessons/neural-networks}}
        ...
        \begin{frame}{Word2vec}
            \begin{figure}[ht!]
                \centering
                \includegraphics[width=0.9\linewidth]{pic/word2vec_arch.png}
                \caption{Word2vec architecture \cite{mikolov2013efficient}.}
            \end{figure}   
        \end{frame}
        ...
        \begin{frame}{Word2vec}
            \begin{figure}[ht!]
                \centering
                \includegraphics[width=0.9\linewidth]{pic/word2vec_action.png}
                \caption{The Illustrated Word2vec.}
                \vspace{-15pt}
                \caption*{\tiny From: \url{https://jalammar.github.io/illustrated-word2vec/}}
                % %%% \label{fig:word2vec_operation} 
            \end{figure}   
        \end{frame}
        ...
        \begin{frame}{Word2vec in action}   
            \begin{figure}[ht!]
                ...
                \vspace{-15pt}
                \caption*{\tiny From: \url{https://doi.org/10.1371/journal.pone.0231189.g008}}
            \end{figure}
        ...
        \begin{frame}{Cosine similarity}   
            \begin{figure}[ht!]
                \centering
                \includegraphics[width=1.0\linewidth]{pic/cosine_similarity_vectors.jpg}
                \caption{Cosine similarity interpretation.}
                \vspace{-15pt}
                \caption*{\tiny From: \url{https://www.learndatasci.com/glossary/cosine-similarity/}}
                % %%% \label{fig:word2vec_operation} 
            \end{figure}
    ...
    \subsection{Shallow embeddings}
        \begin{frame}{Graph embedding}
            It is a function $f: A \rightarrow Z$ that maps each node of the graph $G = (V, E)$ into a $d$-dimensional vector. Where, $Z = \{z_1, z_2, ..., z_N\}$, $z_i \in \mathbb{R}^{d}$ with $d \ll |V|$. % Usually, $z_i$ and $Z$ are called node embedding and embedding matrix, respectively.
        ...
        \begin{frame}{More about embeddings}
            \begin{figure}[ht!]
                ...
                \caption{Visual representation of the three different levels of granularity in graphs. Adapted from \cite{stamile2021graph}.}
            \end{figure}
                        
        \end{frame}

        \begin{frame}{Edge-edge embeddings}
            The set of edge embeddings $\hat{Z} = \{\hat{z}_1, \hat{z}_2, ..., \hat{z}_{|E|}\}$ is calculated for each pair of nodes $(v_i, v_j) \in E$ according to their node embeddings using the operators, such that $e_i \in E$ has its own $\hat{z}_i \in \hat{Z}$. % \cite{stamile2021graph}.

            \begin{itemize}
                \item Concatenate: $z_i \oplus z_j$
                \item Sum: $z_i + z_j$
                \item Average: $\frac{z_i + z_j}{2}$
                \item Hadamard: $z_i \ast z_j$
                \item Weighted-L1: $\vert z_i - z_j\vert$
                \item Weighted-L2: $\vert z_i - z_j\vert^2$
            \end{itemize}
        ...
        \begin{frame}{Graph-level embeddings}
            The representation is calculated using the previous node embeddings $Z = \{z_1, z_2, ..., z_N\}$, $z_i \in \mathbb{R}^{d}$. Thus, it simply aggregates the node embeddings in the (sub) graph by the following operation called global pooling or graph-level readout. % \citep{labonne2023hands}.
                    
            \begin{itemize}
                \item Mean global pooling: $\frac{1}{|V|} \sum_{i=1}^{|V|}z_{i}$
                \item Max global pooling: $\max_{i=1}^{|V|}(z_{i})$
                \item Sum global pooling: $\sum_{i=1}^{|V|}z_{i}$
            \end{itemize}
        ...
        \begin{frame}{Tasks visualization}
            \begin{figure}[ht!]
                \centering
                \includegraphics[width=0.43\linewidth]{pic/graph_learning_tasks_example.png}
                \caption{Examples of graph learning tasks. Adapted from \cite{labonne2023hands}.}
            \end{figure}
        \end{frame}
        ...
        \begin{frame}{Taxonomy of graph machine learning methods}
            \begin{figure}[ht!]
                ...
                \caption{Taxonomy of graph machine learning. Adapted from \cite{chami2022machine}.}
            \end{figure}               
        \end{frame}
        ...
        \begin{frame}{Application on biomedical data} 
            \begin{figure}[ht!]
                ...
                \caption{Applying graph embedding methods to biomedical tasks. Adapted from \cite{yue2020graph}.}
            \end{figure}
        \end{frame}
        ...
        \begin{frame}{Practical example: Node2vec}
            \begin{itemize}
                ...
                \item Lab
                    \begin{itemize}
                        \item              
                            \href{https://drive.google.com/file/d/1FkXYVMxq9TH_57hfl3nBMGsVH9fwZ6dC/view?usp=sharing}{
                                \beamergotobutton{\faGoogle\ Open in Google Colab}
                            }
                    \end{itemize}
            \end{itemize}
    ...
    \subsection{Graph Neural Network (GNN)}
        \begin{frame}{Graph Neural Networks (GNN)}
            It is a function $f: (A, X) \rightarrow Z$ that maps each node of the graph $G = (V, E)$ into a $d$-dimensional vector. Where, $Z = \{z_1, z_2, ..., z_{|V|}\}$, $z_i \in \mathbb{R}^{d}$ with $d \ll |V|$.
            % \newline
        ...
        \begin{frame}{How GNNs works?}
            \begin{figure}[ht!]
                \centering
                \includegraphics[width=0.9\linewidth]{pic/computacional_graph_gcn.png}
                \caption{Computation graph representation.}
                \vspace{-15pt}
                \caption*{\tiny From: \url{https://www.researchgate.net/figure/Given-an-input-graph-a-GNN-predicts-the-label-of-the-target-node-eg-the-blue-node_fig2_346143176}}
                % %%% \label{fig:word2vec_operation} 
            \end{figure}
        \end{frame}
        ...
        \begin{frame}{Computacional graphs}
            \begin{figure}[ht!]
                \centering
                \includegraphics[width=0.8\linewidth]{pic/gnn_computacional_graphs.jpg}
                \caption{Example of computacional graph.}
                \vspace{-15pt}
                \caption*{\tiny From: \url{https://web.stanford.edu/class/cs224w/}}
            \end{figure}
        \end{frame}

        \begin{frame}{Single Layer of GNN}
            \begin{figure}[ht!]
                \centering
                \includegraphics[width=1.0\linewidth]{pic/gnn_single_layer.jpg}
                \caption{Overview of single layer of GNN.}
                \vspace{-15pt}
                \caption*{\tiny From: \url{https://web.stanford.edu/class/cs224w/}}
            \end{figure}
        \end{frame}
        ...
        \begin{frame}{Graph Convolutional Network (GCN)}
            \begin{figure}[ht!]
                \centering
                \includegraphics[width=0.7\linewidth]{pic/graph_neural_networks}
                \caption{GCN scheme.}
            \end{figure}
        ...
        \begin{frame}{Practical example: GCN to node classification} 
            \begin{itemize}
                ...
                \item Lab
                    \begin{itemize}
                        \item              
                            \href{https://drive.google.com/file/d/19ByHxX1AsqLiV3uqIoW5KLiuEDgCWdA5/view?usp=sharing}{
                                \beamergotobutton{\faGoogle\ Open in Google Colab}
                            }
                    \end{itemize}
            \end{itemize}
...
\begin{frame}
    \begin{center}
        ...
        \url{https://www.linkedin.com/in/win7eam/} \\
